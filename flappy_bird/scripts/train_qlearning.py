...
import os
import shutil # 引入 shutil 用于文件操作和目录删除
...
def get_moving_avgs(arr, window, convolution_mode):
    ...
    # Matplotlib 后端可能未加载，但为了完整性保留绘图逻辑
    return np.convolve(
...
def test_with_video_recording(agent):
    """
    循环运行，直到跑出 300 分以上的成绩才停止，并录制视频。
    """
    target_score = 300
    print(f"\nSearching for a high score run (> {target_score}) with video recording...")
    ...
    while True:
        ...
        current_video_folder = f"./videos/qlearning_attempt_{attempt}"
        
        try:
            # 1. 创建环境，使用 render_mode="rgb_array" 进行视频捕获
            env = gym.make("FlappyBirdEnvWithCustomedObs", render_mode="rgb_array")
            ...
            env = gym.wrappers.RecordVideo(
                ...
                name_prefix="qlearning_run"
            )
        except Exception as e:
            print(f"Error creating env: {e}")
            agent.epsilon = old_epsilon # 恢复 epsilon
            return
        ...
        if total_reward > target_score:
            ...
            # 为了方便你下载，把最好的视频复制到 videos 根目录
            best_video_path = f"./videos/BEST_QAGENT_SCORE_{int(total_reward)}.mp4"
            try:
                # 找到生成的 mp4 文件 (通常是 qlearning_run-episode-0.mp4)
                src_file = os.path.join(current_video_folder, "qlearning_run-episode-0.mp4")
                # 确保 videos 目录存在
    ...
    agent.epsilon = old_epsilon # 确保在退出循环前恢复 epsilon
...
if __name__ == "__main__":
    ...
    # 使用新的视频录制函数代替原来的 test_with_render
    test_with_video_recording(agent)
